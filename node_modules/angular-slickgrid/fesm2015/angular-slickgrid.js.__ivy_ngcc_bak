...
import { ComponentFactoryResolver, ApplicationRef, Injector, Injectable, Optional, Input, Component, EventEmitter, ChangeDetectorRef, ElementRef, Inject, Output, NgModule } from '@angular/core';
...
import Flatpickr__default, {  } from 'flatpickr';
...
const numericSorter = (value1, value2, sortDirection, sortColumn) => {
...
const stringSorter = (value1, value2, sortDirection, sortColumn) => {
    if (sortDirection === undefined || sortDirection === null) {
        sortDirection = SortDirectionNumber.neutral;
    }
    let position = 0;
 ...
 */
function convertHierarchicalViewToParentChildArrayByReference(hierarchicalArray, outputArrayRef, options, treeLevel = 0, parentId) {
    const childrenPropName = options && options.childrenPropName || 'children';
    const identifierPropName = options && options.identifierPropName || 'id';
        ...
        return new Promise((resolve, reject) => {
            ...
            setTimeout(() => __awaiter(this, void 0, void 0, function* () {
                try {
                    ...
                    const excelBlob = yield Builder.createFile(this._workbook, { type: 'blob', mimeType });
...
let CompoundDateFilter = class CompoundDateFilter {
    ...
    buildDatePickerInput(searchTerm) {
        ...
        this.flatInstance = ($filterInputElm[0] && typeof $filterInputElm[0].flatpickr === 'function') ? $filterInputElm[0].flatpickr(this._flatpickrOptions) : Flatpickr__default($filterInputElm, this._flatpickrOptions);
...
class InputMaskFilter extends InputFilter {
    ...
    init(args) {
        ...
        // step 3, subscribe to the input change event and run the callback when that happens
        // also add/remove "filled" class for styling purposes
        this.$filterElm.on('keyup input', (e) => {
            let value = '';
            ...
            if (optionCollection.every(x => typeof x === 'string')) {
                ...
                optionCollection.forEach((option) => {
                    ...
                    if (isRenderHtmlEnabled) {
                        // sanitize any unauthorized html tags like script and others
                        // for the remaining allowed tags we'll permit all attributes
                        const sanitizedText = (DOMPurify$1.sanitize(optionText, sanitizedOptions) || '').toString();
            ...
            if (gridOptions && gridOptions.rowDetailView) {
                if (!this._addon) {
                    ...
                    if (!gridOptions.rowDetailView.preTemplate) {
                        ...
                        gridOptions.rowDetailView.preTemplate = () => DOMPurify$2.sanitize(`<div class="${PRELOAD_CONTAINER_PREFIX}"></div>`);
                    }
                    if (!gridOptions.rowDetailView.postTemplate) {
                        ...
                        gridOptions.rowDetailView.postTemplate = (itemDetail) => DOMPurify$2.sanitize(`<div class="${ROW_DETAIL_CONTAINER_PREFIX}${itemDetail[this.datasetIdPropName]}"></div>`);
                    }
...
class AutoCompleteEditor {
    ...
    renderCustomItem(ul, item) {
        ...
        // sanitize any unauthorized html tags like script and others
        // for the remaining allowed tags we'll permit all attributes
        const sanitizedTemplateText = (DOMPurify$3.sanitize(templateString, {}) || '').toString();
        return $('<li></li>')
            .data('item.autocomplete', item)
            .append(sanitizedTemplateText)
            .appendTo(ul);
    ...
    renderCollectionItem(ul, item) {
        ...
        // sanitize any unauthorized html tags like script and others
        // for the remaining allowed tags we'll permit all attributes
        const sanitizedText = (DOMPurify$3.sanitize(finalText, {}) || '').toString();
        const $liDiv = $('<div></div>')[isRenderHtmlEnabled ? 'html' : 'text'](sanitizedText);
        return $('<li></li>')
            .data('item.autocomplete', item)
            .append($liDiv)
...
const flatpickr = Flatpickr; // patch for rollup
...
class DateEditor {
    ...
    applyValue(item, state) {
        ...
        if (fieldName !== undefined) {
            ...
            // validate the value before applying it (if not valid we'll set an empty string)
            const validation = this.validate(state);
            const newValue = (state && validation && validation.valid) ? moment$5(state, outputTypeFormat).format(saveTypeFormat) : '';
            // set the new value to the item datacontext
            if (isComplexObject) {
                setDeepValue(item, fieldName, newValue);
            }
    ...
    isValueChanged() {
        ...
        const outputTypeFormat = mapMomentDateFormatWithFieldType((this.columnDef && (this.columnDef.outputType || this.columnEditor.type || this.columnDef.type)) || FieldType.dateUtc);
        const elmDateStr = elmValue ? moment$5(elmValue, inputFormat, false).format(outputTypeFormat) : '';
        const orgDateStr = this._originalDate ? moment$5(this._originalDate, inputFormat, false).format(outputTypeFormat) : '';
    ...
    serializeValue() {
        ...
        const value = moment$5(domValue, inputFormat, false).format(outputTypeFormat);
        return value;
    }
    validate(inputValue) {
        const isRequired = this.columnEditor.required;
...
const DOMPurify$4 = DOMPurify_; // patch to fix rollup to work
/**
        ...
        if (collection.every(x => typeof x === 'string')) {
            ...
            collection.forEach((option) => {
                ...
                if (isRenderHtmlEnabled) {
                    // sanitize any unauthorized html tags like script and others
                    // for the remaining allowed tags we'll permit all attributes
                    const sanitizedText = (DOMPurify$4.sanitize(optionText, sanitizedOptions) || '').toString();
                    optionText = htmlEncode(sanitizedText);
                }
                // html text of each select option
                let optionValue = option[this.valueName];
...
function getAssociatedDateFormatter(fieldType, defaultSeparator) {
    ...
    return (_row, _cell, value, columnDef, _dataContext, grid) => {
        ...
        const isDateValid = moment$6(value, inputDateFormat, false).isValid();
...
const boldFormatter = (row, cell, value) => {
...
const DOMPurify$5 = DOMPurify_; // patch to fix rollup to work
/**
...
let SlickEmptyWarningComponent = class SlickEmptyWarningComponent {
    ...
    showEmptyDataMessage(isShowing = true, options) {
        ...
        if (!this._warningLeftElement && gridCanvasLeftElm && gridCanvasRightElm) {
            ...
            // sanitize any unauthorized html tags like script and others
            // for the remaining allowed tags we'll permit all attributes
            const sanitizedText = (sanitize(warningMessage, sanitizedOptions) || '').toString();
...
export { Aggregators, AngularSlickgridComponent, AngularSlickgridModule, AngularUtilService, AutoTooltipExtension, BindingEventService, BsDropDownService, CaseType, CellExternalCopyManagerExtension, CellMenuExtension, CheckboxSelectorExtension, CollectionService, ColumnPickerExtension, ContextMenuExtension, DelimiterType, DraggableGroupingExtension, Editors, EmitterType, ExcelExportService, ExportService, ExtensionName, ExtensionService, ExtensionUtility, FieldType, FileType, FilterConditions, FilterFactory, FilterMultiplePassType, FilterService, Filters, Formatters, GraphqlService, GridEventService, GridMenuExtension, GridOdataService, GridService, GridStateService, GridStateType, GroupItemMetaProviderExtension, GroupTotalFormatters, GroupingAndColspanService, HeaderButtonExtension, HeaderMenuExtension, KeyCode, OdataQueryBuilderService, OperatorType, PaginationService, ResizerService, RowDetailViewExtension, RowMoveManagerExtension, RowSelectionExtension, SharedService, SlickPaginationComponent, SlickgridConfig, SortDirection, SortDirectionNumber, SortService, Sorters, TreeDataService, addToArrayWhenNotExists, addWhiteSpaces, arrayRemoveItemByIndex, castToPromise, charArraysEqual, convertHierarchicalViewToParentChildArray, convertHierarchicalViewToParentChildArrayByReference, convertParentChildArrayToHierarchicalView, decimalFormatted, deepCopy, destroyObjectDomElementProps, executeBackendCallback, executeBackendProcessesCallback, exportWithFormatterWhenDefined, findItemInHierarchicalStructure, findOrDefault, formatNumber, getDescendantProperty, getHtmlElementOffset, getTranslationPrefix, htmlDecode, htmlEncode, htmlEncodedStringWithPadding, htmlEntityDecode, htmlEntityEncode, mapFlatpickrDateFormatWithFieldType, mapMomentDateFormatWithFieldType, mapOperatorByFieldType, mapOperatorToShorthandDesignation, mapOperatorType, onBackendError, parseBoolean, parseUtcDate, refreshBackendDataset, sanitizeHtmlToText, setDeepValue, thousandSeparatorFormatted, titleCase, toCamelCase, toKebabCase, toSnakeCase, uniqueArray, uniqueObjectArray, unsubscribeAllObservables, AvgAggregator as ɵa, MinAggregator as ɵb, CompoundInputPasswordFilter as ɵba, CompoundSliderFilter as ɵbb, DateRangeFilter as ɵbc, InputFilter as ɵbd, InputMaskFilter as ɵbe, InputNumberFilter as ɵbf, InputPasswordFilter as ɵbg, MultipleSelectFilter as ɵbh, SelectFilter as ɵbi, NativeSelectFilter as ɵbj, SingleSelectFilter as ɵbk, SliderFilter as ɵbl, SliderRangeFilter as ɵbm, centerFormatter as ɵbn, alignRightFormatter as ɵbo, arrayObjectToCsvFormatter as ɵbp, arrayToCsvFormatter as ɵbq, boldFormatter as ɵbr, bsDropdownFormatter as ɵbs, checkboxFormatter as ɵbt, checkmarkFormatter as ɵbu, complexObjectFormatter as ɵbv, collectionFormatter as ɵbw, collectionEditorFormatter as ɵbx, getAssociatedDateFormatter as ɵby, deleteIconFormatter as ɵbz, MaxAggregator as ɵc, decimalFormatter as ɵca, dollarFormatter as ɵcb, dollarColoredFormatter as ɵcc, dollarColoredBoldFormatter as ɵcd, editIconFormatter as ɵce, fakeHyperlinkFormatter as ɵcf, hyperlinkFormatter as ɵcg, iconFormatter as ɵch, infoIconFormatter as ɵci, italicFormatter as ɵcj, lowercaseFormatter as ɵck, maskFormatter as ɵcl, multipleFormatter as ɵcm, percentFormatter as ɵcn, percentCompleteFormatter as ɵco, percentCompleteBarFormatter as ɵcp, percentSymbolFormatter as ɵcq, progressBarFormatter as ɵcr, translateFormatter as ɵcs, translateBooleanFormatter as ɵct, treeFormatter as ɵcu, uppercaseFormatter as ɵcv, yesNoFormatter as ɵcw, avgTotalsFormatter as ɵcx, avgTotalsDollarFormatter as ɵcy, avgTotalsPercentageFormatter as ɵcz, SumAggregator as ɵd, maxTotalsFormatter as ɵda, minTotalsFormatter as ɵdb, sumTotalsFormatter as ɵdc, sumTotalsBoldFormatter as ɵdd, sumTotalsColoredFormatter as ɵde, sumTotalsDollarFormatter as ɵdf, sumTotalsDollarBoldFormatter as ɵdg, sumTotalsDollarColoredFormatter as ɵdh, sumTotalsDollarColoredBoldFormatter as ɵdi, getAssociatedDateSorter as ɵdj, numericSorter as ɵdk, objectStringSorter as ɵdl, stringSorter as ɵdm, AutoCompleteEditor as ɵe, CheckboxEditor as ɵf, DateEditor as ɵg, DualInputEditor as ɵh, FloatEditor as ɵi, IntegerEditor as ɵj, LongTextEditor as ɵk, MultipleSelectEditor as ɵl, SelectEditor as ɵm, SingleSelectEditor as ɵn, SliderEditor as ɵo, TextEditor as ɵp, executeMappedCondition as ɵq, booleanFilterCondition as ɵr, collectionSearchFilterCondition as ɵs, numberFilterCondition as ɵt, stringFilterCondition as ɵu, testFilterCondition as ɵv, AutoCompleteFilter as ɵw, CompoundDateFilter as ɵx, CompoundInputFilter as ɵy, CompoundInputNumberFilter as ɵz };
